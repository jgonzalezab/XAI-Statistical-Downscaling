...
# Initialize figure
fig, axes = plt.subplots(3, 4, figsize = (16, 8)) # 12
...
cax = fig.add_axes([0.22, 0.08, 0.2, 0.02])
...
for i in range(3):
    ...
    if i == 0:
        compMetric = deltaEnsemble_Future_1 - deltaGCM_Future_1
        compMetricDL = deltaEnsemble_Future_1
        compMetricGCM = deltaGCM_Future_1
    elif i == 1:
        compMetric = deltaEnsemble_Future_2 - deltaGCM_Future_2
        compMetricDL = deltaEnsemble_Future_2
        compMetricGCM = deltaGCM_Future_2
    elif i == 2:
        compMetric = deltaEnsemble_Future_3 - deltaGCM_Future_3
        compMetricDL = deltaEnsemble_Future_3
        compMetricGCM = deltaGCM_Future_3
    ...
    dictMeans = utils.computeRegionMeans(compMetric, regions)
    dictMeansDL = utils.computeRegionMeans(compMetricDL, regions)
    dictMeansGCM = utils.computeRegionMeans(compMetricGCM, regions)
    for region in regions:
        axes[i, 2].text(dictCoord[region][0],
                        dictCoord[region][1],
                        s = r'$\bf{' + str(np.round(dictMeansDL[region]-dictMeansGCM[region], 2)) + '}$',
                        fontsize = 10,
                        horizontalalignment = 'center',
                        verticalalignment = 'center',
    ...
    if i == 0:
        axes[i, 2].set_title('Delta Change Diffs',
                             fontsize = 18)
...
for i in range(3):
    ...
    axes[i, 3] = northAmerica.plot(ax = axes[i, 3],
                                    add_ocean = False,
                                    add_label = False,
                                    line_kws = dict(linewidth = 1),
                                    coastlines = False)
    ...
    dictMeans =  utils.computeRegionMeans(compMetric, regions)
    for region in regions:
        axes[i, 3].text(dictCoord[region][0],
                        dictCoord[region][1],
                        s = r'$\bf{' + str(dictMeans[region]) + '}$',
                        fontsize = 10,
                        horizontalalignment = 'center',
    ...
    if i == 0:
        axes[i, 3].set_title('Standard dev. of ensemble',
                                fontsize = 18)
    axes[i, 3].set_aspect('equal')
    map = Basemap(ax = axes[i, 3],
                    llcrnrlon = -164.75, llcrnrlat = 11.75,
                    urcrnrlon = -59.75, urcrnrlat = 69.75,
                    resolution = 'c')
    map.drawcoastlines(linewidth = 0.6)
    ...
    cax = fig.add_axes([0.6, 0.08, 0.2, 0.02])
...
plt.savefig(FIGS_PATH + 'figEnsemble_v2.pdf',
